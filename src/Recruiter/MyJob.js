...
function MyJob() {
    ...
    const fetchJobs = () => {
        ...
        const recDetails = JSON.parse(localStorage.getItem('userInfo'))
        fetch(base_uri + "/api/all_jd/" + recDetails["recruiterId"]).then((res) => res.json()).then((resp) => {
            console.log(resp)
            setCompName(recDetails["companyName"])
            setData(resp);
            setLoading(false);
        });
    }
                    ...
                    {data.map((jobs) => {
                        return (
                            <div class="col-lg-4 p-2">
                                <Card sx={{ display: 'flex' }} id={jobs.jobId}>
